...
public:
	...
	bool uninitSDK();
	//登录SDK初始化
	long loginSDKInit(CMonitorDatabase::struSqliteData struSqlInfo);
	//登出SDK
	...
	//根据文件类型、时间查找设备录像文件。
	long findFile(const long &lUserID, QDateTime beginDatetime, QDateTime endDatetime, const long &lFileChannel);
	//逐个获取查找到的文件信息

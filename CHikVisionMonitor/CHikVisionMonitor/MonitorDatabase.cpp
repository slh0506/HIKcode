...
bool CMonitorDatabase::insertValues(struSqliteData struSqlInfo)//数据库插入信息
{
	db = QSqlDatabase::addDatabase("QSQLITE", "device_sqlite");
	db.setDatabaseName("deviceDB");
	/*db.database("device_sqlite");*/
...
void CMonitorDatabase::replaceValues(struSqliteData struSqlInfo,const int &id)//数据库替换信息
{
	QSqlQuery query(db);
	db.database("device_sqlite");
	if (!db.open())
